...
namespace Lomo {
    public class ProcedureLaunch : ProcedureBase {
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner) {
            ...
            GameEntry.Resource.LoadAsset("Assets/GameMain/Skills/Skill.asset",
                new LoadAssetCallbacks((assetName, asset, duration, userData) => {
                        Log.Debug("Success {0}", assetName,duration);
                        var skill = asset as SkillBaseGraph;
                        if (skill != null) {
                            Log.Debug("Node Count:{0}", skill.nodes.Count);
                            SkillEngine.Manager.I.StartSkill(skill).OnFinish((_) => {
                                Log.Debug("做完了哦");
                            });
                        }
                    },
                    (assetName, status, errorMessage, userData) => { Log.Info("Fail",errorMessage); }));
        ...
        protected override void OnUpdate(ProcedureOwner procedureOwner, float elapseSeconds, float realElapseSeconds) {
            ...
            // Log.Warning("[P.Update]");
            // if (times < 1) {
            //     TestUpdate.I.StartOrder();
            // }
            //
            // times++;
        }
